...
function gameRowToApi(row) {
    ...
    return {
        ...
        isCompletableViaSCEByProfile: row.is_completable_via_sce_by_profile ? JSON.parse(row.is_completable_via_sce_by_profile) : {},
        isCompletableViaSCEdoublonByProfile: row.is_completable_via_sce_doublon_by_profile ? JSON.parse(row.is_completable_via_sce_doublon_by_profile) : {},
        isCompletableViaSCEwobudgetByProfile: row.is_completable_via_sce_wobudget_by_profile ? JSON.parse(row.is_completable_via_sce_wobudget_by_profile) : {},
...
function cardRowToApi(row) {
    ...
    return {
        ...
        get owner() {
            return Object.entries(this.qtyByProfile).filter(([, q]) => q > 0).map(([p]) => p).join(',');
        },
    };
}
